...
```
이번 주, 두 개의 OS 부품을 동시에 빌드했습니다.

하나는 *나*를 위한 것 — 매일 아침 8시, 텔레그램 봇이 제가 평소 남긴 행동 데이터를 바탕으로 회상 질문을 던집니다. "꿈이 뭐예요" 같은 추상 질문이 아니라, 제가 vault에 남긴 단어·패턴에서 길어 올린 구체적 질문. 5분 답이 6개월 누적되면 그게 곧 자기소개서·면접 자료가 됩니다.

다른 하나는 *팀*을 위한 것 — K-Expo 운영진과 동료가 브라우저 버튼 한 번으로 K-소비재 해외 진출 인사이트 콘텐츠 4,000자를 만드는 Vercel 웹앱. 제 Claude Code 슬래시 커맨드는 저만 쓸 수 있었지만, 이건 비개발자 동료도 쓸 수 있습니다.

빌드하면서 든 생각 세 가지:

1) 정말 *개인화된 서비스를 직접 만들어 쓰는 시대*가 됐습니다. 실무를 해보고 후배에게 업무를 위임해본 사람이 AI 도움을 받으면, 그 일을 *제대로* 자동화할 수 있는 도구를 직접 만들 수 있게 됐습니다.

2) 단, *모르면 뭘 만들어야 할지 엄두가 안 납니다*. 도구 만드는 능력보다 *문제를 알아보는 능력*이 먼저. 새 분야에 계속 호기심 갖고 공부해야겠다는 생각이 더 강해졌습니다.

3) 팀의 업무를 변화시켜 나가는 단계입니다. AI 도움으로 더 빠르게 개선·적응할 수 있을 것 같아 기대됩니다.

#AI #ClaudeCode #업무자동화 #마케팅자동화 #K마케팅 #K엑스포
```

#### Threads 글 (캐주얼 톤)

```
이번 주에 OS 부품 두 개 만들었음.

하나는 내가 매일 5분만 답하면 6개월 후 내 자기소개서가 되는 텔레그램 봇.
다른 하나는 우리 팀 동료가 브라우저 버튼 한 번 누르면 K-Expo 콘텐츠 1편 나오는 웹앱.

만들면서 든 생각:
- 진짜 *내가 직접 만들어 쓰는 시대*. 실무 해본 사람이 AI랑 같이 만들면 제대로 만들어짐
- 근데 *모르면 뭘 만들어야 할지 엄두도 안 남*. 결국 호기심·공부가 먼저
- 팀 업무도 이 흐름으로 바꿔갈 수 있을 것 같아서 기대됨
```

...
![](attachments/vupercent-os-slides.png)
...
1. **통점은 "없음"이 아니라 "흩어짐"이다.** 콘텐츠 아이디어가 없는 게 아니라, 있는데 어디 적지 않으면 사라진다. OS의 역할은 창작이 아니라 **이미 일어난 일을 붙잡는 것**이다.

2. **첫 화면이 전부다.** 기능을 다 넣으면 아무도 안 쓴다. 한 화면에 한 가지만. 나머지는 클릭하고 나서 보여줘도 늦지 않는다.

3. **데모가 있으면 설명이 필요 없다.** "텔레그램 봇이 질문하고 카드뉴스를 만든다"를 설명하는 것보다 URL 하나 보내는 게 100배 빠르다. **OS도 프로토타입이 먼저다.**

4. **전환 목표를 입력값으로 받아라.** 카드뉴스가 아무리 좋아도 마지막 슬라이드의 CTA가 없으면 팔로우에서 끝난다. 전환 설계를 콘텐츠 생성 단계에 함께 넣는 것이 핵심이었다.

5. **팀 안의 레퍼런스가 최고 자산이다.** 오웬(회고 봇), 흐민(Sullivan OS) — 같은 텔레그램+옵시디언 구조를 다른 목적으로 쓰는 레퍼런스를 조 안에서 바로 확인할 수 있었다. 벤치마킹 비용이 0이었다.

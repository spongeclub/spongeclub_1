...
![[KakaoTalk_20260612_122543229_01.jpg|334]]
![[KakaoTalk_20260612_122543229.jpg|337]]
...
const MEGA = {
  math: {
    pinpoint: [
      {emoji:"🎯",title:"[공수1 쪽집게] 현우진 기말 내신집중 패키지",desc:"행렬·경우의수 단원 직접 대비 · 기말고사 전용",url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=52477&MAKE_FLG=1&tec_cd=woojinmath",hot:true},
  ...
  soc: {
    ...
    concept:[
      {emoji:"📺",title:"이다지도 확실한 개념완성 1&2",desc:"22개정 전 단원 개념 완성 강의",url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=56489&MAKE_FLG=&tec_cd=hellohw2"},
    ]
  },
  sci: {
    pinpoint:[
      {emoji:"🎯",title:"[통합과학 쪽집게] 장풍 개념통과 기말 대비",desc:"기말·중간 문제 각 3회분 수록 · 실전 직결",url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=55512&tec_cd=wkdtjdrb",hot:true},
    ...
    concept:[
      {emoji:"📺",title:"베테랑의 통합과학1 개념완성+완자",desc:"개념→완자→문제풀이 3단계",url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=55974&MAKE_FLG=&tec_cd=kodori15th"},
    ]
  },
  kor: {
    pinpoint:[
...
const getWeekDates=()=>{const d=[];const b=new Date();for(let i=0;i<14;i++){const x=new Date(b);x.setDate(b.getDate()+i);d.push(x);}return d;};
        ...
        <div className="page">
          ...
          <div className="card">
            ...
            {[
              ...
              <div key={i} style={{display:"flex",gap:"9px",padding:"8px 0",borderBottom:i<4?"1px solid rgba(255,255,255,.04)":"none",alignItems:"flex-start"}}>
          ...
          <div className="card">
            ...
            {[
              ...
              <div key={i} style={{display:"flex",gap:"9px",padding:"8px 0",borderBottom:i<3?"1px solid rgba(255,255,255,.04)":"none",alignItems:"center"}}>
        ...
        <div className="page">
          ...
          <div style={{background:"rgba(99,102,241,.07)",border:"1px solid rgba(99,102,241,.18)",borderRadius:"12px",padding:"13px",marginTop:"4px"}}>
            ...
            {["강의 1편 보면 → 바로 문제 5문항 풀기 (보기만 하면 소용없음)","주말 집중 수강 → 평일 인강 가능한 자제","쪽집게 강의 먼저 → 개념 강의는 모를 때만","통합사·과 쪽집게가 지금 가장 급함"].map((t,i)=>(
              <div key={i} style={{fontSize:"11px",color:"#6b7280",padding:"4px 0",borderBottom:i<3?"1px solid rgba(255,255,255,.03)":"none"}}>{`▸ ${t}`}</div>
            ))}
          </div>
        </div>
      )}
        ...
        <div className="page">
          ...
          <div className="date-row">
            {weekDates.map((d,i)=>{
              ...
              const isE=d>=EXAM_DATE&&d<=new Date(2026,6,7);
              return(
                <div key={i} className={`date-pill${selDate===i?" on":""}`} style={isE?{background:"rgba(239,68,68,.14)",borderColor:"rgba(239,68,68,.28)"}:{}} onClick={()=>setSelDate(i)}>
                  ...
                  {dt.length>0&&<span style={{fontSize:"8px",color:dn===dt.length?"#6ee7b7":"#6366f1",display:"block"}}>{dn}/{dt.length}</span>}
                  {isE&&<span style={{fontSize:"8px",color:"#fca5a5",display:"block"}}>시험</span>}
                </div>
              );
            })}
          </div>
          <div className="card">
            <div className="ct">
              ...
              {dayTasks.length>0&&<span className={`bdg ${doneToday===dayTasks.length?"bdg-g":"bdg-i"}`} style={{marginLeft:"auto"}}>{doneToday}/{dayTasks.length}</span>}
            </div>
            {dayTasks.length===0?(
              ...
              <div key={i} className="tl-item">
                ...
                <div>
                  ...
                  <div style={{fontSize:"13px",fontWeight:"700",color:item.done?"#374151":item.hot?"#fca5a5":"#e2e8f0",textDecoration:item.done?"line-through":""}}>
                    {item.title}{item.hot&&<span style={{marginLeft:"6px",fontSize:"10px",color:"#ef4444"}}>← 지금!</span>}
                  </div>
...
하지만 구현을 하면서 가장 크게 느낀 것은, 공부 OS는 자료를 많이 모으는 시스템이 아니라 **아이가 숨 쉴 수 있게 공부의 우선순위를 덜어주는 시스템**이어야 한다는 점이었다.
...
공부를 더 시키는 OS가 아니라, **오늘 진짜 해야 할 것 3개만 남겨주는 OS**가 되어야 했다.
### 최종 구현 결과물

최종 구현한 결과물은 **여고 1학년 학생을 위한 학습 루틴 OS**이다.
...
const MEGA_DATA = {
  math:[
    {type:"pinpoint",hot:true,emoji:"🎯",color:"#fca5a5",bg:"rgba(239,68,68,.1)",border:"rgba(239,68,68,.28)",
     ...
     url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=52477&MAKE_FLG=1&tec_cd=woojinmath"},
    {type:"pinpoint",hot:false,emoji:"🎯",color:"#f9a8d4",bg:"rgba(236,72,153,.09)",border:"rgba(236,72,153,.2)",
     ...
     url:"https://www.megastudy.net/lecmain/mainh/mainh.asp"},
    {type:"file",emoji:"📄",color:"#a5b4fc",bg:"rgba(99,102,241,.08)",border:"rgba(99,102,241,.18)",
     ...
     url:"https://www.megastudy.net/teacher_v2/teacher_main.asp"},
    {type:"concept",emoji:"📺",color:"#6b7280",bg:"rgba(255,255,255,.03)",border:"rgba(255,255,255,.07)",
     ...
     url:"https://m.megastudy.net/mobile/smart/lecmain/mainh/unitSearch/kebooks/main.asp"},
  ],
  soc:[
    {type:"pinpoint",hot:true,emoji:"🎯",color:"#fca5a5",bg:"rgba(239,68,68,.1)",border:"rgba(239,68,68,.28)",
     title:"[통합사회 쪽집게] 벼락치기 특강 〈1학기 기말〉",desc:"기말 직전 핵심만 압축 · 지금 바로 수강",
     url:"http://m.megastudy.net/mobile/smart/lecture/detail/view.asp?CHR_CD=54094"},
    {type:"pinpoint",hot:false,emoji:"🎯",color:"#a5b4fc",bg:"rgba(99,102,241,.09)",border:"rgba(99,102,241,.2)",
     ...
     url:"https://m.megastudy.net/mobile/smart/main.asp?TabNo=7"},
    {type:"file",emoji:"📄",color:"#a5b4fc",bg:"rgba(99,102,241,.08)",border:"rgba(99,102,241,.18)",
     title:"[자료파일] 이다지 통합사회 학습자료",desc:"개념 요약 PDF · 단원별 키워드 정리 파일",
     url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=56489&MAKE_FLG=&tec_cd=hellohw2"},
    {type:"concept",emoji:"📺",color:"#6b7280",bg:"rgba(255,255,255,.03)",border:"rgba(255,255,255,.07)",
  ...
  sci:[
    {type:"pinpoint",hot:true,emoji:"🎯",color:"#fca5a5",bg:"rgba(239,68,68,.1)",border:"rgba(239,68,68,.28)",
     ...
     url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=55512&tec_cd=wkdtjdrb"},
    {type:"pinpoint",hot:false,emoji:"🎯",color:"#7dd3fc",bg:"rgba(14,165,233,.09)",border:"rgba(14,165,233,.2)",
     ...
     url:"https://m.megastudy.net/teacher_v2/main.asp?tec_cd=woomaria"},
    ...
    {type:"concept",emoji:"📺",color:"#6b7280",bg:"rgba(255,255,255,.03)",border:"rgba(255,255,255,.07)",
     ...
     url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=55974&MAKE_FLG=&tec_cd=kodori15th"},
  ],
  kor:[
    {type:"pinpoint",hot:true,emoji:"🎯",color:"#fca5a5",bg:"rgba(239,68,68,.1)",border:"rgba(239,68,68,.28)",
    ...
    {type:"file",emoji:"📄",color:"#fcd34d",bg:"rgba(245,158,11,.07)",border:"rgba(245,158,11,.15)",
     ...
     url:"https://m.megastudy.net/mobile/smart/teacher/main.asp?domCd=1"},
    {type:"concept",emoji:"📺",color:"#6b7280",bg:"rgba(255,255,255,.03)",border:"rgba(255,255,255,.07)",
  ...
  eng:[
    ...
    {type:"file",emoji:"📄",color:"#6ee7b7",bg:"rgba(16,185,129,.06)",border:"rgba(16,185,129,.14)",
     ...
     url:"https://m.megastudy.net/mobile/smart/teacher/main.asp?domCd=5"},
    {type:"concept",emoji:"📺",color:"#6b7280",bg:"rgba(255,255,255,.03)",border:"rgba(255,255,255,.07)",
  ...
  hist:[
    {type:"pinpoint",hot:false,emoji:"🎯",color:"#fb923c",bg:"rgba(251,146,60,.08)",border:"rgba(251,146,60,.2)",
     ...
     url:"https://www.megastudy.net/teacher_v2/chr/lecture_detailview.asp?CHR_CD=53206&MAKE_FLG=1&tec_cd=mp1204"},
    {type:"concept",emoji:"📺",color:"#6b7280",bg:"rgba(255,255,255,.03)",border:"rgba(255,255,255,.07)",
     title:"메가 고1 한국사 강좌 보기",desc:"한국사 탭 → 기말 내신 강의 선택",
     url:"https://m.megastudy.net/mobile/smart/teacher/main.asp?domCd=9"},
  ],
};
...
const EBS_DATA = {
  math:[
    {hot:true,emoji:"🆓",color:"#6ee7b7",bg:"rgba(16,185,129,.1)",border:"rgba(16,185,129,.25)",
     ...
     url:"https://www.ebsi.co.kr/ebs/pot/potn/retrieveTchrSubMain.ebs?tabIdx=1"},
    {hot:true,emoji:"🆓",color:"#6ee7b7",bg:"rgba(16,185,129,.08)",border:"rgba(16,185,129,.2)",
     ...
     url:"https://www.ebsi.co.kr/ebs/lms/subMain/subMain.ebs?cookieGradeVal=high1"},
    {hot:false,emoji:"📋",color:"#a5b4fc",bg:"rgba(99,102,241,.07)",border:"rgba(99,102,241,.18)",
     ...
     url:"https://www.ebsi.co.kr/ebs/xip/xipc/previousPaperList.ebs?targetCd=D100"},
  ...
  kor:[
    ...
    {hot:false,emoji:"📚",color:"#fcd34d",bg:"rgba(245,158,11,.07)",border:"rgba(245,158,11,.18)",
     ...
     url:"https://www.ebsi.co.kr/ebs/pot/potn/retrieveTchrSubMain.ebs"},
    {hot:false,emoji:"📋",color:"#a5b4fc",bg:"rgba(99,102,241,.07)",border:"rgba(99,102,241,.18)",
  ...
  eng:[
    ...
    {hot:false,emoji:"🎧",color:"#6ee7b7",bg:"rgba(16,185,129,.07)",border:"rgba(16,185,129,.15)",
     ...
     url:"https://www.ebsi.co.kr/ebs/pot/potg/retrieveMp3DownList.ebs"},
    {hot:false,emoji:"📋",color:"#a5b4fc",bg:"rgba(99,102,241,.07)",border:"rgba(99,102,241,.18)",
...
export default function App(){
  ...
  const toggleHabit=(id)=>{
    ...
    if(!day[id]){
      ...
      if(streak>0&&streak%7===0)setModal({emoji:"🔥",title:`${h.name} ${streak}일 연속!`,sub:"대단해요! 습관이 굳어지고 있어요 💪"});
    }
  };

  const getStreak=(id,log)=>{let s=0;const d=new Date();for(let i=0;i<30;i++){const k=dateKey(new Date(d-i*86400000));if(log[k]?.[id])s++;else break;}return s;};
  const getMonthDone=(id)=>{let c=0;for(let i=0;i<30;i++){const k=dateKey(new Date(Date.now()-i*86400000));if(habitLog[k]?.[id])c++;}return c;};
      ...
      {modal&&(
        <div className="mbg" onClick={()=>setModal(null)}>
          <div className="mm" onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:"42px",marginBottom:"9px"}}>{modal.emoji}</div>
            <div style={{fontSize:"16px",fontWeight:"700",color:"#c7d2fe",marginBottom:"6px"}}>{modal.title}</div>
            <div style={{fontSize:"12px",color:"#6b7280",lineHeight:"1.6",whiteSpace:"pre-line"}}>{modal.sub}</div>
            <button className="mmb" onClick={()=>setModal(null)}>확인 ✓</button>
          </div>
        </div>
      )}
        ...
        <div className="page">
          ...
          <div className="card">
            ...
            {[
              ...
              <div key={i} style={{display:"flex",gap:"9px",padding:"7px 0",borderBottom:i<3?"1px solid rgba(255,255,255,.04)":"none",alignItems:"center"}}>
          ...
          {openSrc==="mega"&&(
            <>
              <div style={{background:"rgba(234,179,8,.07)",border:"1px solid rgba(234,179,8,.2)",borderRadius:"11px",padding:"11px 13px",marginBottom:"12px",fontSize:"11px",color:"#fde047",lineHeight:"1.6"}}>
                ⚡ D-{dday} — 🎯 쪽집게 먼저 / 📄 자료파일 다운 / 📺 개념 강의<br/>
              ...
              <div key={h.id} className="habit-block">
                ...
                <div className="habit-grid">
                  ...
                  {(() => {
                    ...
                    for(let pad=0;pad<startDay;pad++) cells.push(<div key={`p${pad}`} className="hcell" style={{opacity:0}}/>);
                    for(let i=13;i>=0;i--){
      ...
      {tab==="planner"&&(
        <div className="page">
          ...
          <div className="card">
            <div className="ct">
              {weekDates[selDate].getMonth()+1}월 {weekDates[selDate].getDate()}일({DAYS_KO[weekDates[selDate].getDay()]}) 목표
              {dayTasks.length>0&&<span className={`b ${doneToday===dayTasks.length?"bg":"bi"}`} style={{marginLeft:"auto"}}>{doneToday}/{dayTasks.length}</span>}
            </div>
            {dayTasks.length===0?(
              ...
              <div key={i} className="tli">
                ...
                <div>
                  ...
                  <div style={{fontSize:"12px",fontWeight:"700",color:item.hot?"#fca5a5":"#e2e8f0"}}>
                    {item.t}{item.hot&&<span style={{marginLeft:"6px",fontSize:"10px",color:"#ef4444"}}>D-{dday}</span>}
                  </div>
                </div>
              </div>
            ))}
              ...
              {[...wrongs].reverse().map((w,i)=>(
                <div key={w.id||i} className="wc">
                  ...
                  <div className="wcb">
                    ...
                    {w.content.memo&&<div style={{fontSize:"11px",color:"#6b7280",marginTop:"5px",padding:"6px 9px",background:"rgba(99,102,241,.06)",borderRadius:"7px"}}>💬 {w.content.memo}</div>}
                  </div>
                </div>
              ))}
            </>
...
[https://www.instagram.com/p/DZCEqk4GtUu/?igsh=MXc2cG93OTJnMGFuaQ==](https://www.instagram.com/p/DZCEqk4GtUu/?igsh=MXc2cG93OTJnMGFuaQ==)     [https://www.threads.com/@coffeemoms/post/DZCFAx0k6_4?xmt=AQG0z5bq8rfsmxvjcqlZO-jt5mOZJ-PpC5eazl1jPSALzTx0958jyIp-ePDPdPIvzLBDAnDO&slof=1](https://www.threads.com/@coffeemoms/post/DZCFAx0k6_4?xmt=AQG0z5bq8rfsmxvjcqlZO-jt5mOZJ-PpC5eazl1jPSALzTx0958jyIp-ePDPdPIvzLBDAnDO&slof=1)
